...
export const sendOtpEmail = async (to, otp, reason = "verify your email") => {
  try {
    ...
    // this is the actual proof the SMTP relay accepted (or rejected) the message -
    // messageId is what you search for in Brevo's activity log
    console.log(`[mailer] sent to ${to} | messageId=${info.messageId} | response="${info.response}"`);
    return info;
  } catch (err) {
